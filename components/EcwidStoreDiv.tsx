...
export default function EcwidStoreDiv({ storeId = "109087793", baseUrl = '/store', className = '' }: EcwidStoreDivProps) {
  ...
  const initializeEcwid = useCallback(() => {
    if (!document.getElementById('ecStoreProductBrowser')) {
      console.log('EcwidStoreDiv: Store div not found, skipping initialization');
      return;
    }
    ...
    const checkAndInitialize = () => {
      if (document.getElementById('ecStoreProductBrowser') && !initialized.current) {
        try {
          ...
          console.error('Failed to initialize Ecwid:', error);
        }
      }
    };
